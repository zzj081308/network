...
到这里WLAN就搭建完成了，手机和笔记本都能连上wifi，WLAN部分到此为止，eNSP的bug太多，跑路了

## Telnet

有一种方式可以用你的电脑远程登录到设备，不用再跑机房了，没错，就是telnet

借助telnet，直接远程连接设备，还可以在你的电脑上用python写自动化脚本

### eNSP内部登录

eNSP也可以进行telnet远程

下面尝试用SW3登录SW5（因为PC不支持telnet）
...
SW5
[~SW5]aaa
[~SW5-aaa]local-user huawei password cipher Zzj123456@
[*SW5-aaa]local-user huawei service-type telnet 
[*SW5-aaa]local-user huawei ?
[*SW5-aaa]local-user huawei level 3
[*SW5-aaa]quit
[*SW5]user-interface vty 0 4
[*SW5-ui-vty0-4]authentication-mode aaa
[*SW5-ui-vty0-4]commit
```

然后再SW3登录

```
<SW3>telnet 192.168.10.1
Username:huawei
Password:Zzj123456@
```

![](pic/Snipaste_2026-08-14_00-35-21.png)
...
添加一个网卡，一个udp，但是这里我的网卡好像和外部PC撞ip（192.168.100.1）了

改一下网卡，改到192.168.200.1，重启电脑

![](pic/Snipaste_2026-08-14_00-53-46.png)

双向通道勾选

![](pic/Snipaste_2026-08-14_01-00-57.png)

把云和SW5的g1/0/4连接

![](pic/Snipaste_2026-08-14_00-43-21.png)

给SW5的g1/0/4接口配同一个网段的ip：192.168.200.10/24

```
[~SW5]int g1/0/4
[~SW5-GE1/0/4]undo portswitch 
[*SW5-GE1/0/4]ip add 192.168.200.10 24
[~SW5-GE1/0/4]undo shutdown 
[*SW5-GE1/0/4]commit
```

我的电脑pingSW5（192.168.200.10）

![](D:\网络拓扑\network comprehensive\pic\Snipaste_2026-08-14_01-10-10.png)

可通

关闭windows防火墙

![](pic/Snipaste_2026-08-14_01-11-13.png)

SW5ping我的网卡

也可以通

![](pic/Snipaste_2026-08-14_01-12-04.png)
...
![](pic/Snipaste_2026-08-14_01-13-16.png)

成功登录SW5！

有一个python库叫telnetlib，可以在python里控制telnet登录设备

甚至可以写脚本来配置设备

### Telnet脚本控制

写脚本说白了就是模拟手动操作，这里不多说，直接上py代码

但注意用到的库是telnetlib3，telnetlib在python3.13后好像不让用了
...
![](pic/Snipaste_2026-08-14_11-50-38.png)

附python代码文件：[test.py](network comprehensive/test.py)
